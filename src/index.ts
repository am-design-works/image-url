...
export const IMAGE_CDN_BASE = 'https://images.amoeba.works';
...
export const STORAGE_RENDER_PATH = '/storage/v1/render/image/public';

/** Placeholder used when neither bucket nor path is set. Matches droplet's historic value. */
export const PLACEHOLDER_IMAGE_URL = '/images/placeholder-product.png';
 ...
 */
export function publicImageUrl(bucket: string | null | undefined, path: string | null | undefined): string {
  if (!path || !path.trim()) return PLACEHOLDER_IMAGE_URL;
  const b = bucket && bucket.trim() ? bucket.trim() : DEFAULT_BUCKET;
  const p = stripLeadingSlash(path.trim());
  return `${IMAGE_CDN_BASE}${STORAGE_PUBLIC_PATH}/${b}/${p}`;
}
...
export function renderImageUrl(
  ...
  const b = bucket && bucket.trim() ? bucket.trim() : DEFAULT_BUCKET;
  const p = stripLeadingSlash(path.trim());
  const base = `${IMAGE_CDN_BASE}${STORAGE_RENDER_PATH}/${b}/${p}`;
 ...
 */
export function withRenderParams(canonicalUrl: string, opts: RenderOptions): string {
  ...
  if (
    (opts.width || opts.height || opts.quality || opts.format) &&
    u.pathname.includes(STORAGE_PUBLIC_PATH)
  ) {
    u.pathname = u.pathname.replace(STORAGE_PUBLIC_PATH, STORAGE_RENDER_PATH);
  }
 ...
 */
export function isCanonicalImageUrl(url: string | null | undefined): boolean {
  if (!url) return false;
 ...
 */
export function parseImageUrl(url: string | null | undefined): { bucket: string; path: string } | null {
  ...
  if (!KNOWN_STORAGE_HOSTS.has(u.hostname.toLowerCase())) return null;
 ...
 */
export function canonicalizeImageUrl(url: string | null | undefined): string | null {
...
const CANONICAL_RE =
  /^https:\/\/images\.amoeba\.works\/storage\/v1\/(?:object|render\/image)\/public\/[^/]+\/.+$/;

function stripLeadingSlash(s: string): string {
  return s.startsWith('/') ? s.slice(1) : s;
}

function buildQuery(opts: RenderOptions): string {
  const params: string[] = [];
  if (opts.width != null) params.push(`width=${encodeURIComponent(String(Math.round(opts.width)))}`);
  if (opts.height != null) params.push(`height=${encodeURIComponent(String(Math.round(opts.height)))}`);
  if (opts.quality != null) params.push(`quality=${encodeURIComponent(String(Math.round(opts.quality)))}`);
  if (opts.format) params.push(`format=${encodeURIComponent(opts.format)}`);
  return params.join('&');
}

function setIfAbsent(u: URL, key: string, value: number | string | undefined): void {
